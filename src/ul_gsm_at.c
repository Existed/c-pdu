...
u8
at_status_cmp (u8 *s, char *cmp, u16 len)
{
    if (len == 0)
    {
...
u8
at_status_ok (u8 *s, u16 len)
{
    return at_status_cmp (s, "OK", len);
}

u8
at_reply_check_ok (u8 *s)
{
    return at_reply_ok (s, at_reply_len (s)) && at_status_ok (s, 0);
}
