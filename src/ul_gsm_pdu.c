...
void
pdu_hex2bytes (u8 *h, u16 len, u8 *b)
{
    if (len == 0) return;
    len >>= 1;
    while (len--)
...
void
pdu_bytes2hex (u8 *b, u16 len, u8 *h)
{
    if (len == 0) return;
    b += len - 1;
    h += ((u16) len << 1) - 1;
...
pdu_out_encode (struct pdu_struct *in, u8 *out)
{
    ...
    *out++ = in->tp_vp;       // TP-Validity-Period. "AA" means 4 days
    ...
    if (in->tp_dcs == PDU_DCS_UCS2)
    { // UCS2 ? Convert from UTF8
        in->msg.bytes = in->msg.len = (s_utf8_to_ucs2 (in->msg.data,
                                                       in->msg.len,
                                                       (u16 *) (out + 1), 1)
                << 1);
        *out++ = in->msg.len;
...
u16
pdu_out_encode_simple (struct pdu_struct *pdu, u8 *out, void *sender, void *msg,
                       u8 tp_vp)
{
    pdu->first = 0x11;
    pdu->tp_msg_ref = 0;
...
u8
pdu_phone_unpack (u8 *in, u8 len, u8 *out, u8 type)
{
    u8 n = 0;
...
u8
pdu_phone_is_packed (u8 *phone)
{
    return *phone != '+';
}

u8
pdu_phone_digits_bytes (u8 digits)
{
    return (digits + (digits & 1)) >> 1;
}
u8
pdu_phone_bytes (u8 *in)
{
    u8 len = 0;
    while (*in++ != 0xFF)
        len++;
...
u8
pdu_phone_digits (u8 *in, u8 bytes)
{
    u8 len = bytes << 1;
...
u8
pdu_phone_cmp (u8 *in1, u8 in1_bytes, u8 *in2, u8 in2_bytes)
{
